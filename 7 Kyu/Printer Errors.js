/**
* Kata Name : Printer Errors
* Kyu Level : 7 kyu
* Kata Description : In a factory a printer prints labels for boxes. For one kind of boxes
...
const printerError = str => {
    ...
    {
        if(!str[i].match(/^([a-mA-M])$/)) printerErrors++;
    }

    return `${printerErrors}/${str.length}`;
}